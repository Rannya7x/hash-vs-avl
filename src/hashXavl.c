...
int main(){
    for(int i=0;i<4;i++){
        ...
        //Loop de inserção dos dados na Hash/AVL
        t_registro_sensor* sensor; //armazena o registro lido do arquivo
        t_info_consolidada* info = NULL;
        while ((sensor = ler_sensor(medidas))!=NULL){//Enquanto houver registros a serem lidos
            //Geração da chave hash
            char* hora = extrair_hora(sensor->horario);
            int chave = gerar_chave_int(sensor->id, hora);
            /*Busca do MME
            Se a busca retornar NUll, significa que é a primeira medição para aquele sensor na hora
                - add a medição atual como MME na hash/AVL
            Se a busca retornar um valor válido, usamos a MME retornada para calcular a nova MME
                - atualizar a hash/AVL com a nova MME*/
            info = (t_info_consolidada*)hash_buscar(hash, chave);//a hash geralmente é mais rapida
            if(info == NULL){
                t_info_consolidada* info = criar_info_consolidada(chave, sensor->valor, sensor->id, hora);
                double mme_inicial = sensor->valor;
                hash_inserir(hash, chave, info);
                avl_inserir(avl, info);
            }else{
                double mme_suavizada = calcular_mme(info_hash->mme_suavizada, sensor->valor, sensor->alpha);
                info_hash->mme_suavizada = mme_suavizada;
                //hash_atualizar(hash, chave, info);
            }
